...
const _ = {
	...
	interceptors: {
		 ...
		 */
		req(config, count) {
			const fn = count === 1 ? _.interceptors.global.request : this.scopedInterceptors.request
		 ...
		 */
		rep(res, count) {
			const fn = count === 1 ? _.interceptors.global.response : this.scopedInterceptors.response
