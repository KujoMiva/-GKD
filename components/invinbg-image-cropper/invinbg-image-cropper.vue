...
<template>
  <view v-show="show" ref="cropper" class="vue-cropper" :style="{ top : `${containerTop}px` }">
    <view class="cropper-box">
      ...
      >
        <image ref="cropperImg" :src="src" alt="cropper-img" mode="scaleToFill" class="uni-image" />
      </view>
    </view>
    <view class="cropper-drag-box cropper-modal cropper-move pointer-events" />
    <view class="cropper-crop-box" :class="{'pointer-events': cropFixed}" :style="{'width': cropW + 'px','height': cropH + 'px','transform': 'translate3d('+ cropOffsertX + 'px,' + cropOffsertY + 'px,' + '0)'}">
      <view class="cropper-view-box">
        <image :style="{'width': imageWidth + 'px','height': imageHeight + 'px','transform': 'scale(' + scale + ',' + scale + ') ' + 'translate3d('+ (x - cropOffsertX) / scale + 'px,' + (y - cropOffsertY) / scale + 'px,' + '0)' + 'rotateZ('+ rotate * 90 +'deg)'}" mode="scaleToFill" :src="src" alt="cropper-img" />
      </view>

      <view v-if="!cropFixed" class="cropper-face cropper-move" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="cropMoveing" />

      <view class="crop-line line-w" />
      <view class="crop-line line-a" />
      <view class="crop-line line-s" />
      <view class="crop-line line-d" />
      <block v-if="!cropFixed">
        <view class="crop-point point-lt" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'left-top')" />
        <view class="crop-point point-mt" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'middle-top')" />
        <view class="crop-point point-rt" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'right-top')" />
        <view class="crop-point point-ml" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'middle-left')" />
        <view class="crop-point point-mr" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'middle-right')" />
        <view class="crop-point point-lb" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'left-bottom')" />
        <view class="crop-point point-mb" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'middle-bottom')" />
        <view class="crop-point point-rb" @touchstart.stop.prevent="touchStart" @touchmove.stop.prevent="dragMove($event, 'right-bottom')" />
      </block>
    </view>

    <canvas canvas-id="myCanvas" class="cropper-canvas" :style="{ 'width': cropW + 'px','height': cropH + 'px' }" />

    <view class="btn-group">
      <view v-show="showResetBtn" class="btn-item reset-btn" @tap="init" />
      <view v-show="showRotateBtn" class="btn-item rotate-btn" @tap="rotateHandler" />
    </view>
...
export default {
  ...
  methods: {
    ...
    _cropMoveTop(y) {
      this.cropH += this.cropOffsertY - y
      this.cropOffsertY = y
    },
    _cropMoveRight(x) {
    ...
    _cropMoveLeft(x) {
      this.cropW += this.cropOffsertX - x
      this.cropOffsertX = x
    },
    _cropX(x) {
      if (x <= 0) {
      ...
      if (x + this.cropW >= this.windowWidth) {
    ...
    _cropY(y) {
      ...
      if (y + this.cropH >= this.windowHeight) {
        return this.windowHeight - this.cropH - (this.sysInfo.statusBarHeight > 0 ? 0 : 44) - this.border
      }
      return y
    }
  }
